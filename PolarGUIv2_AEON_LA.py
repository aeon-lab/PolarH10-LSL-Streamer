...
Author: Luis Jose Alarcon Aneiva (mods) – base code by m.m.span@rug.nl
...
class BluetoothApp(App):
    ...
    def connect_to_device(self, addr, name, btn):
        # 1 LSL outlet / 6 channels
        info = StreamInfo(name + "_POLAR", "Physio", TOTAL_CH, 0, "float32", addr)
    ...
    def pmd_data_conv(self, _sender, data: bytearray):
        ...
        # ACC frame (52 Hz) – 13 × (x,y,z) int16
            ...
            for off in range(0, len(payload), 6):
                ...
                self.push()  # ECG NaN marks ACC update
    ...
    def hrm_conv(self, _sender, data: bytearray):
        ...
        self.push()   # ECG NaN, HR updated

        # R-R intervals (units: 1/1024 s)
        if flags & 0x10:
            while idx + 1 < len(data):
                self.last_rri = int.from_bytes(data[idx:idx+2], "little") / 1024.0
                idx += 2
                self.push()   # ECG NaN, RRI updated

    # ----------------  Vector assembler  ----------------
    def push(self, ecg=NaN):
        vec = [ecg, self.last_hr, self.last_rri, *self.last_acc]
    ...
    def stop_scanning(self, _):
        ...
        #try:
        #    self.stop_event.set()
        #    # if we created an asyncio loop thread, stop it safely
        #    if hasattr(self, "loop") and self.loop and self.loop.is_running():
        #        self.loop.call_soon_threadsafe(self.loop.stop)
        #except Exception as e:
        #    print(f"[Stop] {e}")
        #finally:
        #    time.sleep(0.1)  # let background callbacks settle
         #   App.get_running_app().stop()
